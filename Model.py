...
class ARCHModel():
    ...
    def _loglikelihood(
        ...
        # 1. Resids
        mp, vp, dp, weight, nu = self._parse_parameters(parameters)
        ...
        llf = self.distribution.loglikelihood(np.append(mp,nu), resids1, resids2, sigma1, sigma2, weight, individual)
        if not individual:
    ...
    def _parse_parameters(
        ...
        return x[:km], x[km : km + kv], [], x[km + kv: km + kv + 1], x[km + kv+1: km + kv + 2]
    ...
    def fit(
        ...
        print("sigma", weight * sigma1 + (1 - weight) * sigma2)
        ...
        names = ["mean1","mean2", "omega", "alpha1", "alpha2","beta1", "beta2", "weight", "nu"]
